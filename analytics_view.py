...
def create_context_chart(row, gymnast_name):
    ...
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False, margin=dict(l=0, r=0, t=30, b=20), height=110, xaxis=dict(range=[x_min, x_max], showgrid=False, dtick=0.5), yaxis=dict(showticklabels=False))
    return fig
...
def show_athlete_history(gymnast_name, selected_meet, color):
    ...
    for _, row in meet_rows.iterrows():
        ...
        # Combined stats line with numerical JSI
        st.markdown(f"""
            <div style='margin-top:-35px; margin-bottom:12px; font-size:0.8rem; color:#888;'>
                🏆 Top {row['Percentile']:.0f}% | <b>Judge's Mood:</b> {get_jsi_label(row['JSI'])} ({row['JSI']:.2f})
            </div>
